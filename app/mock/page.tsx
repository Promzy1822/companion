...
const MOCK_QUESTIONS: Question[] = [
  {id:1,subject:"Mathematics",question:"If 2x + 3 = 11, find x.",options:["2","3","4","5"],correct:2,explanation:"2x = 11-3 = 8, so x = 4"},
  ...
  {id:4,subject:"Chemistry",question:"The atomic number of Carbon is:",options:["6","12","14","8"],correct:0,explanation:"Carbon has 6 protons so atomic number is 6"},
  ...
  {id:7,subject:"English Language",question:"Identify: 'The wind whispered through the trees'.",options:["Simile","Metaphor","Personification","Hyperbole"],correct:2,explanation:"Giving human quality to wind is personification"},
  ...
  {id:9,subject:"Chemistry",question:"Which gas is produced when zinc reacts with dilute H₂SO₄?",options:["Oxygen","Carbon dioxide","Hydrogen","Nitrogen"],correct:2,explanation:"Zn + H₂SO₄ → ZnSO₄ + H₂"},
  {id:10,subject:"Biology",question:"Which blood group is the universal donor?",options:["A","B","AB","O"],correct:3,explanation:"Blood group O can donate to all blood groups"},
];
...
function getUser() {
  if (typeof window === "undefined") return {};
  try { return JSON.parse(localStorage.getItem("companion_user")||"{}"); } catch { return {}; }
}
...
export default function MockExam() {
  ...
  const [mockHistory, setMockHistory] = useState<{score:number;total:number;jambEquivalent:number;date:string}[]>([]);
  ...
  const startExam = async () => {
    ...
    try {
      const prompt = `Generate exactly ${numQuestions} JAMB multiple choice questions for: ${subjects.join(", ")}.
Return ONLY a valid JSON array, no other text:
[{"id":1,"subject":"Mathematics","question":"question text","options":["A text","B text","C text","D text"],"correct":0,"explanation":"why correct"}]
...
- Return ONLY the JSON array`;
  ...
  const submitExam = () => {
    ...
    const res: Result = {score,total:questions.length,bySubject,timeTaken};
    setResult(res);
    setPhase("result");
      ...
      <div style={{padding:"20px 16px"}}>
        <div style={{backgroundColor:cardBg,borderRadius:"20px",padding:"24px",boxShadow:darkMode?"0 2px 8px rgba(0,0,0,0.4)":"0 2px 16px rgba(0,0,0,0.08)",border:`1px solid ${borderC}`,marginBottom:"16px"}}>
          <div style={{textAlign:"center",marginBottom:"24px"}}>
            ...
            <p style={{fontSize:"14px",color:subText,margin:0,lineHeight:"1.5"}}>AI generates questions from your subjects. Full debrief after.</p>
          </div>
    ...
    <div style={{minHeight:"100vh",backgroundColor:bg,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",display:"flex",flexDirection:"column"}}>
      <div style={{background:"linear-gradient(135deg,#431407,#7c2d12,#c2410c)",padding:"16px 20px",position:"sticky",top:0,zIndex:50}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"10px"}}>
          ...
          <div style={{color:"#fff",fontSize:"22px",fontWeight:"900",fontVariantNumeric:"tabular-nums"}}>{formatTime(timeLeft)}</div>
      ...
      <div style={{minHeight:"100vh",backgroundColor:bg,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif"}}>
        ...
        <div style={{padding:"16px",marginTop:"-16px"}}>
              ...
              return (
                <div key={sub} style={{marginBottom:"12px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",fontSize:"13px",marginBottom:"5px"}}>
                    ...
                    <span style={{color:sp>=70?"#16a34a":sp>=50?"#d97706":"#dc2626",fontWeight:"700"}}>{data.correct}/{data.total}</span>
                  </div>
                  <div style={{height:"6px",borderRadius:"3px",backgroundColor:darkMode?"#2c2c2e":"#f0f0f0",overflow:"hidden"}}>
                    <div style={{height:"100%",width:`${sp}%`,borderRadius:"3px",backgroundColor:sp>=70?"#16a34a":sp>=50?"#d97706":"#dc2626"}} />
                  </div>
                </div>
              );
            })}
          ...
          <div style={{backgroundColor:cardBg,borderRadius:"20px",padding:"20px",marginBottom:"16px",boxShadow:darkMode?"0 2px 8px rgba(0,0,0,0.4)":"0 2px 16px rgba(0,0,0,0.08)",border:`1px solid ${borderC}`}}>
            ...
            {result.score===result.total&&<div style={{textAlign:"center",padding:"20px",color:"#16a34a",fontWeight:"700",fontSize:"15px"}}>🎉 Perfect score!</div>}
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
            <button onClick={()=>{setPhase("setup");setResult(null);setQuestions([]);setAnswers({});}} style={{padding:"14px",borderRadius:"14px",border:`1.5px solid ${borderC}`,backgroundColor:"transparent",color:textColor,fontWeight:"700",cursor:"pointer"}}>Try Again</button>
