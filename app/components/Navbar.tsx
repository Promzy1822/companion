      ...
      <div style={{display:"flex", gap:"8px", alignItems:"center"}}>
        ...
        {user ? (
          <div ref={dropRef} style={{position:"relative"}}>
            ...
            {open && (
              <div style={{position:"fixed", top:"70px", right:"16px", width:"240px", backgroundColor:"#fff", borderRadius:"18px", boxShadow:"0 12px 40px rgba(0,0,0,0.2)", border:"1px solid #f0f0f0", overflow:"hidden", zIndex:9999, animation:"dropIn 0.15s ease"}}>
                {/* User info */}
                <Link href="/profile" onClick={()=>setOpen(false)} style={{display:"block", padding:"16px", background:"linear-gradient(135deg,#fff8f5,#fff)", borderBottom:"1px solid #f5f5f5", textDecoration:"none"}}>
                  <div style={{display:"flex", alignItems:"center", gap:"10px"}}>
                    <div style={{width:"42px", height:"42px", borderRadius:"50%", background:"linear-gradient(135deg,#fde68a,#f97316)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"18px", fontWeight:"800", color:"#7c2d12"}}>
                      {user.name.charAt(0).toUpperCase()}
                    </div>
                    <div>
                  ...
                  <div style={{marginTop:"10px", padding:"6px 10px", borderRadius:"8px", backgroundColor:"#fff8f5", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                    <span style={{fontSize:"12px", color:"#666"}}>Target Score</span>
                    <span style={{fontSize:"13px", color:"#ea580c", fontWeight:"800"}}>{user.target} pts</span>
                  </div>
                  <div style={{fontSize:"11px", color:"#ea580c", marginTop:"6px", fontWeight:"600", textAlign:"center"}}>Tap to edit profile →</div>
                </Link>
