...
export default function Auth() {
  ...
  const [form, setForm] = useState({name:"",email:"",password:"",institution:"",course:"",subjects:[] as string[],target:"260",deadline:"",selfRating:"2"});

  useEffect(() => { if (localStorage.getItem("companion_user")) router.replace("/"); }, [router]);
  ...
  useEffect(() => {
    if (form.institution && form.course && form.institution !== "Other" && form.course !== "Other") {
      ...
      setRecommendation({text:getSmartRecommendation(form.institution,form.course), minJamb:cutoff.minJamb, recommendedJamb:cutoff.recommendedJamb, competition:cutoff.competition});
  ...
  const toggleSubject = (s: string) => {
    ...
    if (cur.length >= 3) {
      setError("You can only select 3 subjects. English Language is already included.");
      return;
    }
  ...
  const nextStep = () => {
    ...
    if (step === 2) {
      if (form.subjects.length < 3) { setError("Select exactly 3 more subjects (English is auto-included)"); return; }
      if (form.subjects.length > 3) { setError("You can only select 3 subjects"); return; }
    }
    setStep(s => s+1);
  };
                    ...
                    {prob && (
                      <div style={{padding:"14px 16px",backgroundColor:"#fff"}}>
                        ...
                        <div style={{height:"8px",borderRadius:"4px",backgroundColor:"#f0f0f0",overflow:"hidden"}}>
                          <div style={{height:"100%",width:`${prob.percent}%`,borderRadius:"4px",backgroundColor:prob.color,transition:"width 0.5s ease"}} />
                        </div>
            ...
            {step===2 && (
              <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
                <div>
                  ...
                  <p style={{fontSize:"13px",color:"#999",margin:0}}>English is compulsory. Select exactly 3 more.</p>
                </div>
                <div style={{padding:"12px 14px",borderRadius:"12px",backgroundColor:"#fff8f5",border:"1px solid #fed7aa",display:"flex",alignItems:"center",gap:"8px"}}>
                  <span style={{fontSize:"16px"}}>✅</span>
                  <span style={{fontSize:"13px",color:"#c2410c",fontWeight:"700"}}>English Language — auto included</span>
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:"10px"}}>
                  {ALL_SUBJECTS.map(s => {
                    const sel = form.subjects.includes(s);
                    const maxed = !sel && form.subjects.length >= 3;
                    return (
                      <button key={s} onClick={()=>toggleSubject(s)} style={{
                        padding:"10px 14px",borderRadius:"20px",fontSize:"13px",cursor:maxed?"not-allowed":"pointer",
                        border:sel?"2px solid #ea580c":"1.5px solid #e0e0e0",
                        backgroundColor:sel?"#fff8f5":maxed?"#fafafa":"#fff",
                        color:sel?"#ea580c":maxed?"#ccc":"#555",
                        fontWeight:sel?"700":"400",opacity:maxed?0.5:1,transition:"all 0.15s"
                      }}>{s}</button>
                    );
                  })}
                </div>
                <div style={{padding:"12px 14px",borderRadius:"12px",backgroundColor:form.subjects.length===3?"#f0fdf4":"#fff8f5",border:`1px solid ${form.subjects.length===3?"#86efac":"#fed7aa"}`}}>
                  <div style={{fontSize:"13px",color:form.subjects.length===3?"#16a34a":"#ea580c",fontWeight:"700"}}>
                    {form.subjects.length}/3 selected {form.subjects.length===3?"✓ Perfect! You have your 4 JAMB subjects.":"— need "+(3-form.subjects.length)+" more"}
                  </div>
                  {form.subjects.length===3 && (
                    <div style={{fontSize:"12px",color:"#166534",marginTop:"4px"}}>
                      English + {form.subjects.join(" + ")}
                    </div>
                  )}
                </div>
              </div>
                ...
                {prob && (
                  <div style={{padding:"14px 16px",borderRadius:"14px",border:`1.5px solid ${prob.color}33`,backgroundColor:`${prob.color}08`}}>
                    ...
                    <div style={{height:"10px",borderRadius:"5px",backgroundColor:"#f0f0f0",overflow:"hidden"}}>
                      <div style={{height:"100%",width:`${prob.percent}%`,borderRadius:"5px",backgroundColor:prob.color,transition:"width 0.5s ease"}} />
                    </div>
                  </div>
                )}
