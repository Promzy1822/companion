...
export default function Home() {
  ...
  const fetchNews = useCallback(async (isRefresh = false) => {
    ...
    try {
      ...
      const enriched = (data.news || getFallback()).map((item: NewsItem, i: number) => ({
        ...
        summary: item.title.length > 60 ? item.title.slice(0, 80) + "..." : "Tap to read the full story and stay updated on the latest JAMB news.",
        category: assignCategory(item.title),
      }));
      setNews(enriched);
    } catch {
      setNews(getFallback().map((item, i) => ({ ...item, image: MOCK_IMAGES[i % MOCK_IMAGES.length], summary: "Tap to read the full story.", category: assignCategory(item.title) })));
    } finally {
  ...
  const assignCategory = (title: string): string => {
    ...
    if (/exam|utme|waec|neco|result/.test(t)) return "Exams";
    if (/admission|cutoff|screening|post-utme/.test(t)) return "Admissions";
    if (/tech|ai|digital|online/.test(t)) return "Tech";
    if (/university|polytechnic|college|school/.test(t)) return "Education";
    return "General";
  };
  ...
  const getFallback = (): NewsItem[] => [
    ...
    { title: "WAEC Results 2025: How to Check Online", url: "#", source: "WAEC Nigeria", time: "3d ago" },
    { title: "NUC Approves New Courses in Nigerian Universities", url: "#", source: "Education News", time: "1w ago" },
  ];
  ...
  const calcAggregate = () => {
    ...
    if (calcType === "aggregate") {
      ...
      const jambPart = j / 8, postPart = p / 2, agg = jambPart + postPart;
  ...
  const toggleDark = () => {
    const d = !darkMode; setDarkMode(d); localStorage.setItem("darkMode", String(d));
  };
  ...
  const inputSt: React.CSSProperties = { width:"100%", padding:"13px 16px", borderRadius:"12px", border:`1.5px solid ${borderC}`, fontSize:"15px", outline:"none", backgroundColor: darkMode ? "#2c2c2e" : "#f2f2f7", color:textColor, boxSizing:"border-box", transition:"border-color 0.2s" };
    ...
    <div style={{minHeight:"100vh", backgroundColor:bg, fontFamily:"-apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif", transition:"background 0.3s"}}>
      ...
      <div style={{background:"linear-gradient(160deg, #431407, #7c2d12, #c2410c, #ea580c)", padding:"20px 20px 48px", position:"relative", overflow:"hidden"}}>
        ...
        <div style={{position:"relative"}}>
          <div style={{color:"rgba(255,255,255,0.7)", fontSize:"13px", marginBottom:"4px"}}>Good {new Date().getHours() < 12 ? "morning" : new Date().getHours() < 17 ? "afternoon" : "evening"} 👋</div>
          <div style={{color:"#fff", fontSize:"24px", fontWeight:"800", letterSpacing:"-0.5px"}}>{user?.name.split(" ")[0]}</div>
          <div style={{color:"rgba(255,255,255,0.7)", fontSize:"13px", marginTop:"4px"}}>Target: <span style={{color:"#fde68a", fontWeight:"700"}}>{user?.target} pts</span> • {user?.institution}</div>
        </div>
      </div>
      ...
      <div style={{padding:"0 16px", marginTop:"-28px", paddingBottom:"32px"}}>
        ...
        <Link href="/ai" style={{textDecoration:"none", display:"block", marginBottom:"16px"}}>
          <div style={{background:"linear-gradient(135deg, #c2410c, #ea580c, #fb923c)", borderRadius:"20px", padding:"20px", boxShadow:"0 12px 32px rgba(234,88,12,0.4)", display:"flex", alignItems:"center", gap:"16px", transition:"transform 0.15s", position:"relative", overflow:"hidden"}}>
            <div style={{position:"absolute", top:"-20px", right:"-20px", width:"80px", height:"80px", borderRadius:"50%", background:"rgba(255,255,255,0.08)"}} />
            <div style={{width:"52px", height:"52px", borderRadius:"16px", backgroundColor:"rgba(255,255,255,0.2)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"26px", flexShrink:0}}>🤖</div>
            <div style={{flex:1}}>
              <div style={{color:"#fff", fontWeight:"800", fontSize:"17px", letterSpacing:"-0.3px"}}>Ask AI Anything</div>
              <div style={{color:"rgba(255,255,255,0.8)", fontSize:"13px", marginTop:"3px"}}>Your 24/7 JAMB tutor is ready</div>
            </div>
            <div style={{width:"32px", height:"32px", borderRadius:"50%", backgroundColor:"rgba(255,255,255,0.2)", display:"flex", alignItems:"center", justifyContent:"center", color:"#fff", fontSize:"16px"}}>→</div>
          </div>
        </Link>
          ...
          {[
            ...
            <Link key={i} href={c.href} style={{textDecoration:"none"}}>
              <div style={{backgroundColor:cardBg, borderRadius:"18px", padding:"20px 16px", boxShadow: darkMode ? "0 2px 8px rgba(0,0,0,0.4)" : "0 2px 12px rgba(0,0,0,0.08)", textAlign:"center", border:`1px solid ${borderC}`, transition:"transform 0.15s"}}>
                <div style={{width:"44px", height:"44px", borderRadius:"12px", backgroundColor: `${c.color}18`, display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 10px", fontSize:"22px"}}>{c.icon}</div>
                <div style={{fontWeight:"700", color:c.color, fontSize:"15px"}}>{c.label}</div>
                <div style={{fontSize:"12px", color:subText, marginTop:"3px"}}>{c.sub}</div>
              </div>
            </Link>
          ))}
        </div>
          ...
          {showCalc && (
            <div style={{padding:"0 20px 20px", borderTop:`1px solid ${borderC}`}}>
              ...
              <div style={{display:"flex", flexDirection:"column", gap:"12px"}}>
                ...
                {calcError && <div style={{padding:"10px 14px", backgroundColor: darkMode ? "#2a0000" : "#fff0f0", borderRadius:"10px", color:"#ef4444", fontSize:"13px", fontWeight:"500"}}>⚠️ {calcError}</div>}
                <button onClick={calcAggregate} style={{padding:"14px", borderRadius:"14px", border:"none", background:"linear-gradient(135deg, #c2410c, #ea580c)", color:"#fff", fontWeight:"700", fontSize:"15px", cursor:"pointer", boxShadow:"0 4px 12px rgba(234,88,12,0.3)"}}>
                  Calculate →
                </button>
                {aggResult && (
        ...
        <div style={{backgroundColor:cardBg, borderRadius:"18px", overflow:"hidden", boxShadow: darkMode ? "0 2px 8px rgba(0,0,0,0.4)" : "0 2px 12px rgba(0,0,0,0.08)", border:`1px solid ${borderC}`}}>
          ...
          <div style={{padding:"18px 20px 0"}}>
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"14px"}}>
              ...
              <button onClick={() => fetchNews(true)} style={{width:"32px", height:"32px", borderRadius:"10px", backgroundColor: darkMode ? "#2c2c2e" : "#f2f2f7", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontSize:"15px", transition:"transform 0.3s", transform: refreshing ? "rotate(180deg)" : "rotate(0)"}}>🔄</button>
            </div>

            {/* Category Chips */}
            <div style={{display:"flex", gap:"8px", overflowX:"auto", paddingBottom:"14px", scrollbarWidth:"none", msOverflowStyle:"none"}}>
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => setActiveCategory(cat)} style={{flexShrink:0, padding:"7px 14px", borderRadius:"20px", border:"none", cursor:"pointer", fontSize:"13px", fontWeight:"600", backgroundColor: activeCategory === cat ? "#ea580c" : darkMode ? "#2c2c2e" : "#f2f2f7", color: activeCategory === cat ? "#fff" : subText, transition:"all 0.2s", whiteSpace:"nowrap"}}>
                  {cat}
                </button>
              ))}
            </div>
          ...
          {newsLoading ? (
            ...
            <div style={{padding:"32px", textAlign:"center", color:subText, fontSize:"14px"}}>No {activeCategory} news right now</div>
          ) : (
            <div>
              {filteredNews.map((item, i) => (
                <a key={i} href={item.url} target="_blank" rel="noopener noreferrer" style={{textDecoration:"none", display:"block"}}
                  ...
                  <div style={{
                    padding:"16px 20px",
                    borderTop: i === 0 ? "none" : `1px solid ${borderC}`,
                    display:"flex", gap:"14px", alignItems:"flex-start",
                    backgroundColor: pressedCard === i ? (darkMode ? "#2c2c2e" : "#f5f5f7") : "transparent",
                    ...
                    <div style={{flex:1, minWidth:0}}>
                      ...
                      <div style={{fontSize:"12px", color:subText, lineHeight:"1.5", display:"-webkit-box", WebkitLineClamp:2, WebkitBoxOrient:"vertical", overflow:"hidden"}}>{item.summary}</div>
              ...
              <div style={{padding:"16px 20px", borderTop:`1px solid ${borderC}`, textAlign:"center"}}>
                <button onClick={() => fetchNews(true)} style={{background:"none", border:"none", color:"#ea580c", fontSize:"14px", fontWeight:"700", cursor:"pointer"}}>
