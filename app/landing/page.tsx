...
export default function Landing() {
  ...
  const features = [
    ...
    {icon:"📊", title:"Track Progress", desc:"Study streaks, scores and performance per subject"},
  ...
  return (
    <div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif",backgroundColor:"#fff",overflowX:"hidden"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:"20px"}}>
          {steps.map((s,i)=>(
            <div key={i} style={{display:"flex",gap:"16px",alignItems:"flex-start"}}>
              <div style={{width:"40px",height:"40px",borderRadius:"50%",flexShrink:0,background:"linear-gradient(135deg,#c2410c,#ea580c)",display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:"900",fontSize:"16px",boxShadow:"0 4px 12px rgba(234,88,12,0.3)"}}>
                {s.n}
              </div>
          ...
          {testimonials.map((t,i)=>(
            <div key={i} style={{backgroundColor:"#fff",borderRadius:"16px",padding:"16px",boxShadow:"0 2px 8px rgba(0,0,0,0.06)",border:"1px solid #fed7aa"}}>
              <div style={{display:"flex",gap:"3px",marginBottom:"8px"}}>
                {"⭐⭐⭐⭐⭐".split("").map((s,j)=><span key={j} style={{fontSize:"14px"}}>{s}</span>)}
              </div>
      ...
      <div style={{padding:"48px 24px",background:"linear-gradient(135deg,#7c2d12,#c2410c,#ea580c)",textAlign:"center"}}>
        ...
        <Link href="/auth" style={{display:"block",padding:"18px",borderRadius:"30px",backgroundColor:"#fff",color:"#c2410c",fontWeight:"800",fontSize:"17px",textDecoration:"none",boxShadow:"0 8px 24px rgba(0,0,0,0.2)",marginBottom:"12px"}}>
          Create Free Account →
        </Link>
