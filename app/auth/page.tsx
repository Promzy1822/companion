...
export default function Auth() {
  ...
  const [form, setForm] = useState({ name:"", email:"", password:"", institution:"", course:"", target:"260", deadline:"", selfRating:"2" });
  ...
  const prob = rec && form.institution !== "Other" && form.course !== "Other" ? getAdmissionProbability(parseInt(form.target), getCutoff(form.institution, form.course)) : null;
  ...
  const handleLogin = () => {
    ...
    if (saved) {
      ...
      if (u.email === form.email && u.password === form.password) { router.replace("/"); return; }
    }
    setError("Wrong email or password. Please sign up if new.");
  };
  ...
  const next = () => {
    ...
    if (step === 2 && combo && flex.length < combo.flexibleCount) { setError(`Select ${combo.flexibleCount - flex.length} more subject(s)`); return; }
    setStep(s => s + 1);
  };
  ...
  const signup = () => {
    ...
    const u = { ...form, subjects, cutoffData: cutoff, recommendation: rec?.text || null };
    localStorage.setItem("companion_user", JSON.stringify(u));
    router.replace("/");
  };
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#fff", fontFamily: "-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#7c2d12,#c2410c,#ea580c)", padding: "24px 24px 28px", textAlign: "center", position: "relative" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "10px" }}>
          <img src="/icon-192.png" alt="Companion" width={60} height={60} style={{ borderRadius: "16px", boxShadow: "0 8px 24px rgba(0,0,0,0.3)" }} />
        ...
        {mode === "login" ? (
          ...
          <div>
                  ...
                  <div>
                    ...
                    <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                      {combo.flexible.map((s: string) => {
                        const sel = flex.includes(s); const max = !sel && flex.length >= combo.flexibleCount;
            ...
            <div style={{ marginTop: "20px", display: "flex", gap: "12px" }}>
              ...
              <button onClick={step < 3 ? next : signup} style={{ flex: 2, padding: "14px", borderRadius: "30px", border: "none", background: "linear-gradient(135deg,#c2410c,#ea580c)", color: "#fff", fontWeight: "800", fontSize: "15px", cursor: "pointer", boxShadow: "0 4px 16px rgba(234,88,12,0.35)" }}>
