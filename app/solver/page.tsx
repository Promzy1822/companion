...
function SolverContent() {
  ...
  const [history, setHistory] = useState<
    { q: string; a: string; subject: string; topic?: string }[]
  >([]);
  ...
  const solveQuestion = async () => {
    ...
    const prompt = `You are an expert JAMB examiner. A Nigerian student needs help with this ${
      subject || "JAMB"
    } question${
      year ? ` from ${year} JAMB` : ""
    }${
...
[Give one similar JAMB-style question]`;
    ...
    try {
      ...
      const reply =
        data.reply || "Could not solve. Try again.";
  ...
  const generateQuestion = async () => {
    ...
    const prompt = `Generate one realistic JAMB ${subj} past question${
      year ? ` in the style of ${year}` : ""
    }${
      top ? ` specifically on the topic "${top}"` : ""
    }. This is for a Nigerian student preparing for JAMB UTME.
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background:
            "linear-gradient(135deg,#3b0d02,#7c2d12,#c2410c,#ea580c)",
        ...
        >
          ...
          <div>
            ...
            >
              {topic
                ? `Topic: ${topic}`
                : "AI explains every answer in detail"}
            </div>
          </div>
        </div>
        ...
        >
          ...
          {mode === "type" ? (
            ...
            <>
              ...
              >
                {topic
                  ? `AI will generate a JAMB-style question specifically on "${topic}"`
                  : "Select a subject and generate a realistic JAMB question."}
              </p>
